...
#  if !refresh_cache && File.exist?(cache_file)
  if File.exist?(cache_file)
    ...
    if cached_data && cached_data["input"] == markdown_content
      # hugo comment
      drugged_output = cached_data["output"].inject_in_front_matter("# Generated by Geminock v#{TranslatorVersion} . cache_key='#{cache_key}'\ngeminock_ver: '#{TranslatorVersion}'\n")
      return drugged_output
    end
...
def translate_with_gemini(file_name:, extension:, lang:, output_file:) #  , overwrite: false)
  ...
  if ENABLE_GEMINI
    puts("Note we have new_matter but we dont use it with gemini. i think we should!")
