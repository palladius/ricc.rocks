...
INPUT_SUBFOLDER ||= ENV.fetch('INPUT_SUBFOLDER', '') # 'src/posts'
OUTPUT_FOLDER ||= ENV.fetch('OUTPUT_FOLDER', 'out') # 'out/'
raise "no ENV[GEMINI_API_KEY]!!" if GEMINI_API_KEY.nil?

# Input folder
#input_folder = 'doc/posts'
raise "Give me an ENV[INPUT_BASE_FOLDER]!! For instance src/posts. this is a campaign to sensibilize Riccardo on making this explicit in ENV" unless INPUT_BASE_FOLDER
...
puts(Rainbow("🇬🇵 INPUT_BASE_FOLDER:  #{INPUT_BASE_FOLDER}").cyan)
puts(Rainbow("🇬🇵 INPUT_SUBFOLDER:    #{INPUT_SUBFOLDER}").cyan)
...
# Run the import
# 1. Call gemini
iteration_results = iterate_through(
#  input_folder: INPUT_FOLDER,
  input_base_folder: INPUT_BASE_FOLDER,
  input_subfolder: INPUT_SUBFOLDER,
  extensions:,
  languages:,
  overwrite_existing_out_file: OVERWRITE_EXISTING_OUT_FILES,
  max_articles: MAX_ARTICLES,
  restrict_src_path_by_regex: RESTRICT_SRC_PATH_BY_REGEX,
