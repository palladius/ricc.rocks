# translate_with_gemini(file_name: doc/posts/medium/2022-09-12-GCP-CB-trigger-with-pulumi-python/index.md, extension: zzo, lang: it). MD_content: 3KB. prompt_version=1.0
...
TODO(7feb23): aggiungere questo video come incorporato: https://www.youtube.com/watch?v=5jhWVNN8P_M (creato il 6feb23)
...
Comunque, sono così innamorato di ⬣ GCP (che mi paga lo stipendio, devo ammetterlo), Cloud Build, Cloud Deploy e in generale delle pipeline CI/CD su Google Cloud che volevo provarlo. Cercando su Google "Cloud Build Pulumi" ho trovato questo bell'articolo per JavaScript, che non è nelle mie corde.
...
Il codice consente a qualsiasi progetto Pulumi tu possa avere su GCP (in Python 🐍) di impostare un trigger per l'autoaggiornamento. Un push al repository git attiverà un processo di compilazione che, in caso di successo, effettuerà il login come te a Pulumi ed emetterà un aggiornamento con il nuovo codice.

Quindi, se, ad esempio, si esegue il commit di una modifica che aggiunge un bucket GCS al codice, in un paio di minuti quel bucket GCS verrà creato e il file README.md verrà aggiornato con i parametri del builder:

![cb-trigger-list](02-pulumi-commands.webp)
In grassetto puoi vedere 3 parametri aggiornati da Cloud Build stesso!

Il mio codice supporta **Github** (così com'è) e **Bitbucket** (codice al 99% lì come stava funzionando prima!) per il momento.
...
Ho provato a usare Google Translate, sembra che possa significare scopa (🧹) in hawaiano o birmano (🇲🇲, ora Myanmar). Fino ad allora, userò la prima emoji, finché qualcuno non mi dimostrerà il contrario.
...
##  Prossimi passi
I miei 📝 per il futuro includono:
